...
| 小鹤音形 | `/Users/shendongchun/Documents/rime-origin` | 本地 `10.26.x` 配置 | 原目录未附再分发许可，仅限本机使用；主词典直接复用三个 bin |
| OpenCC `t2s` | `BYVoid/OpenCC` / Squirrel 1.1.2 包 | `ver.1.1.9` / `556ed22496d650bd0b13b6c163be9814637970ae` | 文件摘要见数据锁 |

## 小鹤音形

`rime-origin/build` 的 `flypy.table.bin`、`flypy.prism.bin`、`flypy.reverse.bin` 是主词典唯一权威来源。三者摘要记录在 `DATA_LOCK.json` 和 `docs/FLYPY_MIGRATION_AUDIT.md`。随包 `flypy.dict.yaml` 只有一个哨兵词条，是部署占位文件；应用启动和重新部署后都会把三个权威 bin 逐字节恢复到部署目录，任何临时编译产物都不会参与实际候选。
...
这些 bin 是架构无关的 Rime 数据，不是 Intel App 或动态库；已用 arm64 版 librime 1.16 在 Apple Silicon 上验证加载。
...
3. 更新小鹤音形时，只从用户确认的 `rime-origin/build` 替换三个 bin，同时更新摘要和真实候选回归；不得再从第三方公开词表近似重建主词典。
