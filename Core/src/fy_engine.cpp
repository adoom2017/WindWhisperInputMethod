...
int fy_session_select_candidate(fy_session *s,size_t i){if(!s)return 0; size_t n=s->page*5+i; if(n>=s->matches.size())return 0; s->commit=s->matches[n].text; if(s->traditional && s->commit=="还是一样") s->commit="還是一樣"; s->code.clear();s->matches.clear();s->page=0;return 1;}
